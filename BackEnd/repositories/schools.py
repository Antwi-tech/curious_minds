...
class SchoolDetails:
        ...
        try:
            ...
            for s in slots:
                result.append({
                    ...
                    "company_description": s.company.description,
    ...
    def auto_approve_school(self, school_id: int, confidence: int,
        ...
        try:
            ...
            if school:
                ...
                school.ai_confidence = confidence
                school.ai_decision = decision
                school.ai_reasoning = reasoning
                db.commit()
        except SQLAlchemyError as e:
            db.rollback()
            print(f"Error auto-approving school: {e}")
        finally:
            db.close()

    def save_ai_result(self, school_id: int, confidence: int,
                    decision: str, reasoning: str):
        db = self.get_session()
        try:
            school = db.query(School).filter_by(school_id=school_id).first()
    ...
    def save_ai_description(self, school_id: int, description: str):
        db = self.get_session()
        try:
            school = db.query(School).filter_by(school_id=school_id).first()
            if school and description:
                school.description = description
                db.commit()
                print(f"AI description saved for school {school_id}")
        except SQLAlchemyError as e:
            db.rollback()
    ...
    def save_verification_result(self, school_id: int, result: dict):
        db = self.get_session()
        try:
            school = db.query(School).filter_by(school_id=school_id).first()
            if school:
                school.ai_confidence = result.get("legitimacy_score", 0)
                school.ai_decision = result.get("recommendation", "manual_review").upper()
                school.ai_reasoning = (
                ...
                db.commit()
                print(f"Verification result saved for school {school_id}")
        except SQLAlchemyError as e:
            db.rollback()
